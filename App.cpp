...
App::App(int width, int height) {
    ...
    avl_edit_tablet_->setStyleSheet("QGraphicsView {"
                                    "background: #d5d5d5; }");
    connect(avl_edit_tablet_, &GraphWidget::mousePressedSignal, this, &App::clickedOnTree);
...
//    }
    ...
    rb_edit_tablet_->setStyleSheet("QGraphicsView {"
                                    "background: #d5d5d5; }");
    connect(rb_edit_tablet_, &GraphWidget::mousePressedSignal, this, &App::clickedOnTree);
...
//    }
    ...
    car_edit_tablet_->setStyleSheet("QGraphicsView {"
                                   "background: #d5d5d5; }");
    connect(car_edit_tablet_, &GraphWidget::mousePressedSignal, this, &App::clickedOnTree);
...
//    }
    ...
    splay_edit_tablet_->setStyleSheet("QGraphicsView {"
                                    "background: #d5d5d5; }");
    connect(splay_edit_tablet_, &GraphWidget::mousePressedSignal, this, &App::clickedOnTree);
...
//    }
    ...
    clear_cur_tree_ = new QPushButton(tree_settings_);
    clear_cur_tree_->resize(300, 60);
    clear_cur_tree_->move(20, 200);
    clear_cur_tree_->setFont(QFont(font_family_, 30));
    clear_cur_tree_->setText("Clear all tree");
    connect(clear_cur_tree_, SIGNAL(released()), this, SLOT(clearAllTree()));
...
void App::setCartTree() {
    CarTree::render(car_edit_scene_, back_car_tree_->getRoot());
    start_screen_->hide();
    window_->show();
    avl_window_->hide();
    rb_window_->hide();
...
void App::addSomeNodes() {
    ...
    for (int i = 0; i < cnt; ++i) {
        if (!avl_window_->isHidden()) {
            int64_t data = rnd() % 1000;
            while (back_avl_tree_->contains(data)) data = rnd() % 1000;
            back_avl_tree_->insert(data);
        }
        if (!car_window_->isHidden()) {
            int64_t data = rnd() % 1000;
            while (back_car_tree_->contains(data)) data = rnd() % 1000;
            int64_t prior = rnd() % 1000000;
            back_car_tree_->insert(data, prior);
        }
        if (!splay_window_->isHidden()) {
            int64_t data = rnd() % 1000;
            while (back_splay_tree_->contains(data)) data = rnd() % 1000;
            back_splay_tree_->insert(data);
        }
        if (!rb_window_->isHidden()) {
            int64_t data = rnd() % 1000;
            while (back_rb_tree_->contains(data)) data = rnd() % 1000;
            back_rb_tree_->insert(data);
        }
    }
    if (!avl_window_->isHidden()) AVL::render(avl_edit_scene_, back_avl_tree_->getRoot());
    if (!car_window_->isHidden()) CarTree::render(car_edit_scene_, back_car_tree_->getRoot());
...
void App::clickedOnTree(int x, int y) {
    if (!avl_window_->isHidden()) {
...
void App::clearAllTree() {
    if (!avl_window_->isHidden()) {
        back_avl_tree_->delAllTree();
        AVL::render(avl_edit_scene_, back_avl_tree_->getRoot());
    }
    if (!car_window_->isHidden()) {
        back_car_tree_->delAllTree();
        CarTree::render(car_edit_scene_, back_car_tree_->getRoot());
    }
    if (!rb_window_->isHidden()) {
        back_rb_tree_->delAllTree();
        RBTree::render(rb_edit_scene_, back_rb_tree_->getRoot());
    }
    if (!splay_window_->isHidden()) {
        back_splay_tree_->delAllTree();
        Splay::render(splay_edit_scene_, back_splay_tree_->getRoot());
    }
